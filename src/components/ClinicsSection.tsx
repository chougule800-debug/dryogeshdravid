...
interface ClinicsSectionProps {
  ...
  loading?: boolean;
  error?: string | null;
  onBookBranch: (branchId: ClinicBranchId) => void;
}
...
export const ClinicsSection: React.FC<ClinicsSectionProps> = ({ 
  ...
  // Helper to calculate the next 2nd Sunday of the month for Goa
  const nextSecondSunday = useMemo(() => {
    const today = new Date();
    const findSecondSunday = (year: number, month: number) => {
      let sundaysCount = 0;
    ...
    let targetDate = findSecondSunday(today.getFullYear(), today.getMonth());
    // If 2nd Sunday of current month has already passed, get next month's 2nd Sunday
    if (targetDate && targetDate < today) {
      const nextMonth = today.getMonth() + 1;
      const nextYear = nextMonth > 11 ? today.getFullYear() + 1 : today.getFullYear();
      targetDate = findSecondSunday(nextYear, nextMonth % 12);
    }
    ...
    return 'Second Sunday of Every Month';
  }, []);
              ...
              >
                ...
                <div className={`p-6 sm:p-7 ${
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <span className="text-xs font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-white/20 text-slate-100">
                      {isGoa ? 'Goa Visiting Centre' : 'Main Belgaum Centre'}
                    </span>
                    <span className="text-xs text-slate-100 font-medium flex items-center gap-1">
                ...
                <div className="p-6 sm:p-7 space-y-6 flex-1">
                  {/* Goa Special Schedule Notification Box */}
                  {isGoa && (
                    <div className="p-4 rounded-2xl bg-amber-50/90 border border-amber-300 text-amber-950 space-y-2">
                      ...
                      <p className="text-xs leading-relaxed text-amber-950 font-normal">
                        Consultations in Quepem, South Goa are held strictly <strong className="font-bold">Every Second Sunday (10:00 AM to 2:00 PM)</strong>.
                      </p>
                      <div className="pt-1 flex items-center gap-2 text-xs font-bold text-amber-950">
                        <Calendar className="w-3.5 h-3.5 text-amber-800" />
                        <span>Next Upcoming Goa Visit: <u>{nextSecondSunday}</u></span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    <div className="text-xs font-bold uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
                      ...
                      <span className="text-slate-900 font-bold">Clinic Address</span>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="text-xs font-bold uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
                      ...
                      <span className="text-slate-900 font-bold">Consultation Timings</span>
                    </div>
                    <div className="space-y-1.5">
                      {clinic.timings.map((time, idx) => (
                        <div
                          key={idx}
                          className="flex items-start gap-2 text-xs font-medium text-slate-950 bg-slate-50 p-2.5 rounded-xl border border-slate-200"
                        >
                  ...
                  <div className="space-y-2">
                    <div className="text-xs font-bold uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
                      ...
                      <span className="text-slate-900 font-bold">Contact & Helpline</span>
                    </div>
                    <div className="flex items-center gap-3 flex-wrap">
                      <a
                        href={`tel:${clinic.phone}`}
                        className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-[#1C3F3A] font-bold text-sm border border-emerald-300 transition-colors"
                      >
                      ...
                      <a
                        href={`https://wa.me/918762465349?text=${encodeURIComponent(`Hello Dr. Dravid's Clinic, I would like to enquire about consultation at your ${clinic.city} branch.`)}`}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-green-50 hover:bg-green-100 text-green-950 font-bold text-xs border border-green-300 transition-colors"
                      >
                  ...
                  <div className="rounded-xl overflow-hidden border border-slate-200 bg-slate-100 h-36 relative">
                    ...
                    <div className="absolute bottom-2 right-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 bg-white text-slate-950 rounded-md shadow-xs hover:bg-slate-50 border border-slate-300"
                      >
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
                  <span className="text-xs text-slate-900 font-semibold text-center sm:text-left">
                    {isGoa ? 'Slots fill quickly for 2nd Sundays' : 'Walk-in & scheduled consultations'}
                  </span>

                  <button
                    onClick={() => onBookBranch(clinic.id)}
                    className={`w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-xs sm:text-sm text-white shadow-xs transition-all active:scale-98 cursor-pointer ${
                      isGoa
                        ...
                        : 'bg-[#2D5A50] hover:bg-[#20423a]'
                    }`}
                  >
                    <Calendar className="w-4 h-4 text-emerald-300" />
                    <span>Book at {isGoa ? 'Goa Branch' : 'Belgaum Clinic'}</span>
                  </button>
                </div>
              </div>
            );
