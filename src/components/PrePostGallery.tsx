...
export const PrePostGallery: React.FC<PrePostGalleryProps> = ({
  cases,
  onContactClinic,
  loading,
  error,
  ...
  const getSliderPos = (id: string) =>
    sliderPositions[id] !== undefined ? sliderPositions[id] : 50;
          ...
          {error && (
            <div className="col-span-full p-10 rounded-3xl bg-white border border-slate-200 text-center">
              <p className="text-sm text-slate-500 font-semibold">
                Unable to load clinical results from the server. Please try again.
              </p>
            </div>
          )}
            ...
            return (
              ...
              >
                <div className="relative h-56 sm:h-64 lg:h-72 w-full overflow-hidden bg-slate-900 select-none group">
                  {/* AFTER media layer (full background) */}
                  <div className="absolute inset-0">
                    <MediaRenderer
                      type={caseItem.afterMediaType}
                      url={caseItem.afterMediaUrl}
                      alt={`${caseItem.title} After`}
                      className="absolute inset-0 w-full h-full object-cover"
                    />
                  </div>
                  <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded bg-emerald-950/80 text-emerald-200 text-[8px] sm:text-[10px] font-bold uppercase tracking-wider backdrop-blur-xs z-10 border border-emerald-500/30">
                    After
                  </div>

                  {/* BEFORE media layer (clipped by slider position) */}
                  <div
                    className="absolute top-0 left-0 bottom-0 overflow-hidden"
                    style={{ width: `${pos}%` }}
                  >
                    {/* The inner wrapper is sized to the FULL container width so
                        the media is not squashed when the slider moves. The
                        wrapper's width is a percentage of the parent, and we
                        scale it up by 100/pos. */}
                    <div
                      className="absolute top-0 left-0 h-full"
                      style={{ width: `${100 / (pos / 100)}%` }}
                    >
                      <MediaRenderer
                        type={caseItem.beforeMediaType}
                        url={caseItem.beforeMediaUrl}
                        alt={`${caseItem.title} Before`}
                        className="absolute inset-0 w-full h-full object-cover"
                      />
                    </div>
                    <div className="absolute top-2 left-2 px-1.5 py-0.5 rounded bg-slate-950/80 text-slate-200 text-[8px] sm:text-[10px] font-bold uppercase tracking-wider backdrop-blur-xs z-10 border border-slate-700/50">
                      Before
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    style={{ left: `${pos}%` }}
                  >
                    <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white text-[#2D5A50] shadow-md flex items-center justify-center text-[10px] sm:text-[11px] font-bold border border-slate-300">
                      &#8596;
                    </div>
                  </div>
                ...
                <div className="p-3 sm:p-4 flex items-center justify-between gap-2 bg-white">
                  <div>
                    <span className="text-[8px] sm:text-[10px] font-bold text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-full">
                      {caseItem.category}
                    </span>
                    <h3 className="text-xs sm:text-sm font-bold text-slate-900 mt-1 truncate max-w-[140px] sm:max-w-[200px]">
                      {caseItem.title}
                    </h3>
                  </div>
                  <button
                    onClick={onContactClinic}
                    className="px-2.5 sm:px-3 py-1.5 rounded-full bg-[#2D5A50] hover:bg-[#20423a] text-white text-[10px] sm:text-xs font-semibold shrink-0 cursor-pointer shadow-xs transition-colors"
                  >
                    Contact Clinic
                  </button>
                </div>
